...
# SSL setup (required for pyquotex)
import certifi
...
async def start_streaming(asset_display: str):
    global REALTIME_RUNNING, CURRENT_CANDLE, CANDLES
    if not CLIENT:
        return
    ...
    # Load historical data for 1m to start
...
async def realtime_price_loop(asset_display: str):
    ...
    while REALTIME_RUNNING:
        try:
            ...
            log(f"⚠️ realtime loop error: {e}", level=2)
...
def get_candles():
    ...
    if pair in ASSET_DISPLAY_MAP:
        ...
        internal_pair = DISPLAY_TO_INTERNAL[pair]
    else:
        internal_pair = pair

    market_name = ASSET_DISPLAY_MAP.get(internal_pair, internal_pair)

    # Get candles from memory
    asset_candles = CANDLES.get(internal_pair, {}).get(timeframe, [])
    candles = asset_candles[-count:] if asset_candles else []
...
def startup_background():
    # Wait a bit for Flask to start, then connect
    time.sleep(2)
    ...
    try:
        ...
        if success:
            log("✅ Quotex connected, starting streaming for default asset", level=1)
            # Start streaming for a default asset, e.g. "AUD/CAD (OTC)"
            default = "AUD/CAD (OTC)"
            asyncio.run_coroutine_threadsafe(start_streaming(default), ASYNC_LOOP)
        else:
...
if __name__ == '__main__':
    # When running locally, you can set env vars and run with python app.py
